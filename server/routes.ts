...
import { registerSchema, loginSchema, userPreferencesSchema, insertUserRecipeSchema, insertFeedbackSchema, inviteCodes as inviteCodesTable, type UserPreferences } from "@shared/schema";
import { db } from "./db";
...
Write 1-2 sentences analysing their trend: rate of change, whether they're on track, and one concrete actionable suggestion. Keep it encouraging and practical. Plain text, no markdown.`;
  ...
  // ── Cycle phase daily tip (research-grounded) ─────────────────────────────
  ...
  app.get("/api/cycle/periods", async (req, res) => {
    if (!req.session.userId) return res.status(401).json({ message: "Not authenticated" });
    const logs = await storage.getCyclePeriodLogs(req.session.userId);
    res.json(logs);
  });

  app.post("/api/cycle/periods", async (req, res) => {
    if (!req.session.userId) return res.status(401).json({ message: "Not authenticated" });
    try {
      const body = z.object({
  ...
  app.patch("/api/cycle/periods/:id", async (req, res) => {
    if (!req.session.userId) return res.status(401).json({ message: "Not authenticated" });
    try {
      const id = parseInt(req.params.id);
      const body = z.object({
  ...
  app.delete("/api/cycle/periods/:id", async (req, res) => {
    if (!req.session.userId) return res.status(401).json({ message: "Not authenticated" });
    const id = parseInt(req.params.id);
  ...
  app.get("/api/cycle/insights", async (req, res) => {
    if (!req.session.userId) return res.status(401).json({ message: "Not authenticated" });
  ...
  // ── Cycle phase evidence (research-backed bullets, cached per phase/week) ──

  const phaseEvidenceCache = new Map<string, { data: object; expiresAt: number }>();

  app.get("/api/cycle/phase-evidence", async (req, res) => {
    if (!req.session.userId) return res.status(401).json({ message: "Not authenticated" });
    if (!process.env.OPENAI_API_KEY) return res.status(503).json({ error: "AI unavailable" });
  ...
  // ── AI personalised insights narrative ────────────────────────────────────

  app.get("/api/cycle/ai-insights", async (req, res) => {
    if (!req.session.userId) return res.status(401).json({ message: "Not authenticated" });
    if (!process.env.OPENAI_API_KEY) return res.status(503).json({ error: "AI unavailable" });
  ...
  // ── Research pulse (weekly research summaries per phase) ───────────────────

  const researchPulseCache = new Map<string, { data: object; expiresAt: number }>();

  app.get("/api/cycle/research-pulse", async (req, res) => {
    if (!req.session.userId) return res.status(401).json({ message: "Not authenticated" });
    if (!process.env.OPENAI_API_KEY) return res.status(503).json({ error: "AI unavailable" });

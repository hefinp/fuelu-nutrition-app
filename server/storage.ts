...
export class DatabaseStorage implements IStorage {
  ...
  async getMealPlanGenerationCounts(userId: number, monthKey: string): Promise<{ dailyCount: number; weeklyCount: number }> {
    const [row] = await db.select().from(mealPlanGenerations).where(and(eq(mealPlanGenerations.userId, userId), eq(mealPlanGenerations.monthKey, monthKey)));
    return { dailyCount: row?.dailyCount ?? 0, weeklyCount: row?.weeklyCount ?? 0 };
  }

  async incrementMealPlanGeneration(userId: number, monthKey: string, type: 'daily' | 'weekly'): Promise<{ dailyCount: number; weeklyCount: number }> {
    const col = type === 'daily' ? mealPlanGenerations.dailyCount : mealPlanGenerations.weeklyCount;
    const [row] = await db
      .insert(mealPlanGenerations)
      .values({ userId, monthKey, dailyCount: type === 'daily' ? 1 : 0, weeklyCount: type === 'weekly' ? 1 : 0 })
      .onConflictDoUpdate({
        target: [mealPlanGenerations.userId, mealPlanGenerations.monthKey],
        set: { [type === 'daily' ? 'dailyCount' : 'weeklyCount']: sql`${col} + 1` },
      })
      .returning();

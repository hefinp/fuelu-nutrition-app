...
- `components/cycle-tracker.tsx` — Menstrual cycle tracker (female users only). Shows current phase (menstrual/follicular/ovulatory/luteal), day of cycle, colour-coded phase card with static tip, cycle progress bar, and date/length inputs that auto-save via `PUT /api/user/preferences`. Collapsible "Today's nutrition tip" card (`GET /api/cycle/daily-tip?phase=X`) fetches a GPT-4o tip on demand; cached server-side by phase + date. Route: `GET /api/cycle/daily-tip`.

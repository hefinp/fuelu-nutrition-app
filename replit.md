...
### Strava Integration (Tasks #292, #297, #298)
Users can connect their Strava account via OAuth to pull fitness activity data into the dashboard. The `strava_connections` table stores per-user OAuth tokens (access, refresh, expiry) and athlete ID. The `strava_activities` table persists activity data locally (backfilled 7 days on connect, synced in real-time via webhooks). Backend routes (`/api/strava/*`) handle auth initiation, callback, status check, disconnect, and activity data fetching (DB-backed with API fallback). Webhook endpoints (GET/POST `/api/strava/webhook`) handle Strava event subscriptions for real-time activity create/update/delete sync. Date-specific (`/api/strava/activities/date/:date`) and range (`/api/strava/activities/range?from=&to=`) endpoints serve diary and TDEE integration. The Activity widget (`client/src/components/activity-widget.tsx`) appears in the Tracking column/tab showing recent activities, weekly stats (calories, distance, duration, heart rate), and a connect prompt when Strava is not linked. The food diary (`diary.tsx`) shows a `DiaryActivitySection` per day with exercise details and calories burned when Strava is connected. The adaptive TDEE calculation (`server/lib/adaptive-tdee.ts`) incorporates Strava exercise calories for improved accuracy and confidence scoring. Environment variables: `STRAVA_CLIENT_ID`, `STRAVA_CLIENT_SECRET`, `STRAVA_REDIRECT_URL`, `STRAVA_WEBHOOK_VERIFY_TOKEN`.
